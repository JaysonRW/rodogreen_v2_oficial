...
import { applyHiddenMeshes, printModelHierarchy } from "./truckMaterialOverrides";
...
export function createTruck(gltf: GLTF): TruckRig {
  ...
  printModelHierarchy(gltf.scene);
  applyHiddenMeshes(gltf.scene);
  ...
  baseModel.name = "TruckBaseSurface";
  const bodyModel = new THREE.Mesh(body, materials.rodogreenYellow);
  ...
  const dump = createDumpBody(bodyModel, bounds);
